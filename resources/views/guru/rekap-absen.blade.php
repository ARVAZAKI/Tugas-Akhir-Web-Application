...
<div class="bg-white shadow-lg rounded-lg overflow-hidden mt-8">
    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
        <h2 class="text-xl font-semibold text-gray-800">Rekap Daftar Siswa Hadir</h2>
        <div>
            <span class="text-gray-600 text-sm">{{ $mapel->nama_mapel }} - {{ $kelas->nama_kelas }}</span>
        </div>
    </div>

    <!-- Filter Form -->
    <div class="px-6 py-4 border-b border-gray-200">
        <form method="GET" action="{{ route('rekap-absen-siswa', ['mapelId' => $mapel->id, 'kelasId' => $kelas->id]) }}" class="flex items-center gap-4">
            <div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white">
            ...
            <tbody class="text-gray-700 text-sm font-light">
                ...
                @empty
                    <tr>
                        <td colspan="3" class="text-center py-3">Belum ada siswa hadir</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
